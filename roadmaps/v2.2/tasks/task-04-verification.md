...
- [ ] A table-driven `TestErrorStatusMatrix` asserts, for every error origin, both the HTTP status code AND the structured-JSON envelope shape (`{"error":{"code","message","details","request_id"}}`), with one row per origin:
  - [ ] `extract_error` → 400, code `BAD_REQUEST` (extractor returns a non-`*espresso.Error`).
  - [ ] `extract_espresso_error` → the carried status/code (extractor returns an `*espresso.Error`, e.g. `ErrUnprocessableEntity`).
  - [ ] `handler_error` → the carried status/code (handler returns `ErrNotFound(...)`).
  - [ ] `handler_plain_error` → 500, code `INTERNAL` (handler returns a non-`*espresso.Error`; the unchanged fallback).
  - [ ] `service_validation` → 400, code `VALIDATION_ERROR`, with `details` carrying field errors (Task 2; matches auto-validate-on-extract).
  - [ ] `service_circuit_breaker_open` → 503, code `SERVICE_UNAVAILABLE` (Task 2).
  - [ ] `service_timeout` → 503, code `SERVICE_UNAVAILABLE` (Task 2; this row is the regression-lock for the behavior change called out in Task 2).
  - [ ] `panic` → 500, code `PANIC`, envelope INCLUDES the `details` key (Task 3; previously omitted).
  - [ ] `auth_failure` → 401, code `UNAUTHORIZED`, `Content-Type: application/json` (Task 3; previously text/plain).
  - [ ] `rate_limit` → 429, code `TOO_MANY_REQUESTS`, `Content-Type: application/json` (Task 3; previously text/plain).
- [ ] Every matrix row asserts `Content-Type: application/json; charset=utf-8` and that the body decodes into the envelope struct with a non-empty `code` and `message`; no row asserts on a text/plain body.
- [ ] A table-driven `TestHandlerSignatureMatrix` registers one route per supported handler shape and asserts each produces a 2xx with the expected body:
  - [ ] reflection `func() T`, `func(*Req) T`, `func(ctx, *Req) (T, error)`, and `Service[Req,Res]`.
  - [ ] typed `HandlerCtxReqErr`, `HandlerCtxReq`, `HandlerReqErr`, `HandlerReq`, `HandlerCtxNoErr`.
  - [ ] typed `HandlerCtxReq1Req2Err` / `Lungo` populates BOTH extractors (the shape that works today).
  - [ ] the two-extractor reflection outcome from Task 1: if approach A (extend reflection), a reflection-registered two-extractor route returns 2xx with both extractors populated; if approach B (fail-fast), registration panics with the actionable message and the request-time `panic("espresso: invalid handler argument - this is a bug")` is unreachable.
- [ ] A `TestNoRequestTimeInvalidArgumentPanic` (or equivalent) asserts the `"this is a bug"` panic in `createHandlerFromInfo` (`handler.go` ~744) is never reached at request time for any registered shape.
- [ ] A doc/godoc-vs-code consistency check (`TestDocsConsistency`, scripted grep over `docs/` + exported godoc) fails if a removed symbol or a false signature claim reappears.
...
- [ ] Project coverage does not regress below the established minimum; the new matrices measurably raise coverage of `writeHandlerError`, the service-layer error mapping, and the unified envelope writer.
- [ ] `TestErrorStatusMatrix` includes the `service_timeout` row asserting 503 — confirming (not contradicting) the Task 2 change to `TestWithLayersTyped_WithTimeout`.
- [ ] PR description lists the three matrices and notes that this task adds tests only — no production code changes.
