...
1. **Reflection path** — `Handler(any)` / `router.Handle(any)` / `router.Get/Post/...(path, fn)`. Inspects the function via reflection at registration time, caches results in `handlerCache` (a process-global, **LRU-bounded** cache keyed by `reflect.Type`). Supports `func() T`, `func(*Req) T`, `func(ctx, *Req) (T, error)`, `func(ctx, *Req) T`, and `Service[Req,Res]`. Panics at registration (not request) for invalid signatures. **Two-extractor `func(ctx, *Req1, *Req2) (T, error)` is NOT supported on this path** — `handlerInfo` carries a single request slot, so a second extractor arg is rejected with a panic at *registration* time (`handler.go` `handlerFunc`) naming the typed alternative; use the typed `HandlerCtxReq1Req2Err` / `Lungo` constructor (next bullet) instead.
